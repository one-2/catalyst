...
class TensorOpsTestUsingGPUStrategy : public ::testing::Test {
protected:
    TensorOps* tensorOps;

    void SetUp() override {
        tensorOps = new TensorOps(true); // Using GPU execution for tests
    }
...
TEST_F(TensorOpsTestUsingGPUStrategy, ZeroTensor) {
...
TEST_F(TensorOpsTestUsingGPUStrategy, IdentityTensor) {
...
TEST_F(TensorOpsTestUsingGPUStrategy, SumTensors) {
...
TEST_F(TensorOpsTestUsingGPUStrategy, MultipleTensor) {
    TensorPtr a = std::make_shared<torch::Tensor>(torch::tensor({{1.0, 2.0}, {3.0, 4.0}}, torch::dtype(torch::kFloat64)));
...
TEST_F(TensorOpsTestUsingGPUStrategy, ScalarProduct) {
...
TEST_F(TensorOpsTestUsingGPUStrategy, TensorProduct) {
    TensorPtr a = std::make_shared<torch::Tensor>(torch::tensor({{1.0, 2.0}, {3.0, 4.0}}, torch::dtype(torch::kFloat64)));
    TensorPtr b = std::make_shared<torch::Tensor>(torch::tensor({{5.0, 6.0}, {7.0, 8.0}}, torch::dtype(torch::kFloat64)));
...
TEST_F(TensorOpsTestUsingGPUStrategy, ReLu) {
    TensorPtr a = std::make_shared<torch::Tensor>(torch::tensor({{-1.0, 2.0}, {-3.0, 4.0}}, torch::dtype(torch::kFloat64)));
...
TEST_F(TensorOpsTestUsingGPUStrategy, Softmax) {
    TensorPtr a = std::make_shared<torch::Tensor>(torch::tensor({1.0, 2.0, 3.0}, torch::dtype(torch::kFloat64)));
